// src/app/(admin)/admin/keuangan/page.tsx
import { prisma } from "@/lib/prisma";
import Link from "next/link";
import { revalidatePath } from "next/cache";
import { redirect } from "next/navigation";
import { Wallet, TrendingUp, TrendingDown, Landmark, Receipt, ArrowDownRight, ArrowUpRight, CalendarDays, Search, PieChart, Plus, X, Building2 } from "lucide-react";
...
export default async function KeuanganPage({ searchParams }: { searchParams: Promise<{ start?: string, end?: string, view?: string, modal?: string }> }) {
    ...
    const showModal = params.modal === 'tambah-pengeluaran';

    const today = new Date();
    const firstDayOfMonth = new Date(today.getFullYear(), today.getMonth(), 1);

    const startDateStr = params.start || firstDayOfMonth.toISOString().split('T')[0];
    ...
    // =========================================================================
    // SERVER ACTION: Fungsi untuk Menyimpan Pengeluaran Operasional (OpEx)
    // =========================================================================
    ...
    // =========================================================================
    // 1. DATA KAS MASUK (LABA BERSIH DARI PROYEK LUNAS)
    // =========================================================================
    ...
    const kasMasukData = paidProjects.map(proj => {
    ...
    return (
        <div className="p-4 md:p-8 bg-slate-50 min-h-screen relative">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="bg-slate-900 rounded-3xl p-6 md:p-8 shadow-xl relative overflow-hidden text-white">
                            ...
                            <div className="flex items-center gap-3 mt-4">
                                ...
                                <span className="text-[10px] text-slate-400 font-medium">Laba Bersih Proyek dikurangi Gaji & Operasional</span>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col justify-between">
                                <div>
                                    ...
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Pemasukan (Laba Proyek)</p>
                            ...
                            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                                ...
                                <div className="mt-4 pt-4 border-t border-slate-100 space-y-2">
                                    <div className="flex justify-between items-center text-[10px] md:text-xs">
                                        <span className="text-slate-500 font-medium flex items-center gap-1"><Wallet size={12} /> Operasional: Gaji Karyawan</span>
                                        <span className="font-bold text-black">{formatRp(totalBebanGaji)}</span>
                                    </div>
                                    <div className="flex justify-between items-center text-[10px] md:text-xs">
                                        <span className="text-slate-500 font-medium flex items-center gap-1"><Building2 size={12} /> Operasional: Beban Lainnya</span>
                                        <span className="font-bold text-black">{formatRp(totalBebanPerusahaan)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
                            ...
                            <div className="space-y-4">
                                {kasKeluarData.slice(0, 5).length === 0 ? (
                                    <p className="text-center text-sm text-black italic py-4">Belum ada transaksi.</p>
                                ) : (
                                    kasKeluarData.slice(0, 5).map((tx) => {
                                        const Icon = tx.icon;
                                        return (
                                            <div key={tx.id} className="flex gap-4 items-start p-3 hover:bg-slate-50 rounded-2xl transition-colors">
                                                <div className={`p-2 rounded-xl shrink-0 ${tx.color}`}><Icon size={16} /></div>
                                                <div className="flex-1 min-w-0">
                                                    <p className="font-bold text-slate-900 text-xs md:text-sm truncate">{tx.title}</p>
                                                    <p className="text-[9px] text-black truncate mt-0.5">{tx.subtitle}</p>
                                                </div>
                                                <div className="text-right shrink-0"><p className="font-black text-red-600 text-xs md:text-sm">-{formatRp(tx.amount)}</p></div>
                                            </div>
                                        );
                                    })
                                )}
            ...
            {/* TAB 2: KAS MASUK (Pemasukan Laba Bersih Proyek) */}
            {view === 'masuk' && (
                <div className="animate-in fade-in duration-300">
                    <div className="bg-green-600 text-white p-6 md:p-8 rounded-2xl md:rounded-3xl shadow-xl shadow-green-900/20 mb-6 md:mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0">
                        <div className="w-full sm:w-auto">
                            <p className="text-green-100 text-[10px] md:text-xs font-black uppercase tracking-widest mb-1">Total Laba Bersih Proyek Terfilter</p>
                        ...
                        <div className="flex flex-row items-center w-full sm:w-auto mt-2 sm:mt-0">
                            <div className="bg-white/20 px-4 py-2.5 rounded-xl backdrop-blur-sm border border-green-500/30 w-full text-center">
                                <p className="text-[11px] sm:text-sm md:text-base font-bold whitespace-nowrap">{kasMasukData.length} Proyek Lunas</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="hidden md:block bg-white rounded-2xl md:rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                        <table className="w-full text-sm text-left">
                            <thead className="bg-slate-50 text-slate-600 uppercase text-[10px] tracking-widest">
                                <tr>
                                    <th className="px-6 py-5">Tgl Lunas</th>
                                    <th className="px-6 py-5">Nama Proyek</th>
                                    <th className="px-6 py-5">Nilai Proyek (Kotor)</th>
                                    <th className="px-6 py-5 text-red-600">Beban Proyek</th>
                                    <th className="px-6 py-5 text-right border-l border-slate-200 text-green-700">Laba Bersih Proyek</th>
                                </tr>
                            </thead>
                                ...
                                {kasMasukData.length === 0 ? <tr><td colSpan={5} className="text-center py-8 text-slate-500">Tidak ada pemasukan di periode ini.</td></tr> : kasMasukData.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-50">
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            <p className="text-[10px] text-black">{item.client}</p>
                                        </td>
                                        <td className="px-6 py-4 font-semibold text-black">{formatRp(item.gross)}</td>
                                        <td className="px-6 py-4 font-semibold text-red-500">-{formatRp(item.expense)}</td>
                            ...
                            <div key={item.id} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                                <div className="flex justify-between items-start mb-3 border-b border-slate-100 pb-3">
                                    <div className="flex-1 pr-2">
                                        ...
                                        <p className="text-[10px] text-black mt-1">{item.client} • {item.date.toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
                                    </div>
                                </div>
                                ...
                                <div className="flex justify-between items-center pt-1">
                                    <span className="text-[10px] font-black text-green-700 uppercase">Laba Bersih</span>
            ...
            {view === 'keluar' && (
                <div className="animate-in fade-in duration-300">
                    <div className="bg-red-600 text-white p-5 sm:p-6 md:p-8 rounded-2xl md:rounded-3xl shadow-xl shadow-red-900/20 mb-6 md:mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0">
                        ...
                        {/* PERBAIKAN: Flex Row sejajar di Mobile */}
                    ...
                    <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col">
                        <div className="p-5 md:p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                            <h3 className="font-black text-slate-900 uppercase tracking-widest text-xs md:text-sm flex items-center gap-2">
                                <Building2 size={16} className="text-blue-500" /> Catat OpEx Perusahaan
                            </h3>
                        ...
                        <form action={simpanPengeluaran} className="p-5 md:p-6 flex flex-col gap-4">
                            ...
                            <div>
                                ...
                                <input type="date" name="date" required defaultValue={new Date().toISOString().split('T')[0]} className="w-full px-4 py-2.5 text-black bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-600 outline-none transition-all" />
                            </div>
                            <div>
                                <label className="text-[10px] font-black text-slate-800 uppercase tracking-widest block mb-1">Keterangan / Tujuan</label>
                                <input type="text" name="description" required placeholder="Cth: Iklan Google Ads, Biaya Server, Sewa..." className="w-full px-4 py-2.5 text-black bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-600 outline-none transition-all" />
                            </div>
                            <div>
                                <label className="text-[10px] font-black text-slate-800 uppercase tracking-widest block mb-1">Nominal (Rp)</label>
                                <div className="relative">
                                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 font-bold">Rp</span>
                                    <input type="number" name="amount" required min="1" placeholder="500000" className="w-full pl-10 pr-4 py-2.5 text-black bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-600 outline-none transition-all font-bold" />
                                </div>
                            </div>
                            <button type="submit" className="w-full bg-blue-600 text-white font-black py-3.5 rounded-xl hover:bg-blue-700 transition-all text-xs tracking-widest mt-2 shadow-lg shadow-blue-600/20">
                                SIMPAN OPEX
                            </button>
                        </form>
                    </div>
